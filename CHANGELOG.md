...
- `Core.Imaging`: 連結成分で円盤を 1 件も検出できない場合の **Hough 円検出によるフォールバック**を追加（#91）。連結成分は外周リングが途切れず繋がっていることを前提にするが、印字が薄いと分断されて断片が最小サイズに届かないことがある。Hough は勾配を見るため連結性を要求しない。半径の探索範囲は規格径（123mm / 125mm ±10%）で厳しく絞る。範囲を緩めると誤検出が激増し、実測では円盤 5 枚のシートに対し 15 個を検出した
