...
- `Core.Imaging`: 円盤の中心を bbox 中心から**外周輪郭への楕円フィット**に変更（#91）。しきい値を振ったときの中心のブレは実測で bbox 中心の ±2.4〜5.7px に対し ±0.2〜0.8px と 1 桁改善する。線画の円盤では稀にフィットが外れるため、bbox 中心から直径の 5% を超えて離れた結果は採用しない
