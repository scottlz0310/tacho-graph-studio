...
public sealed record DiscSplitOptions
{
    ...
    public const int DefaultThreshold = 7;
    public const int DefaultPaddingPx = 20;
    ...
    public int PaddingPx { get; init; } = DefaultPaddingPx;
    ...
    public static void Validate(DiscSplitOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);

        if (options.Threshold is < MinThreshold or > MaxThreshold)
        {
            throw new ArgumentException(
                $"Threshold は {MinThreshold}〜{MaxThreshold} で指定してください: {options.Threshold}",
                nameof(options));
        }
        ...
        {
            throw new ArgumentException(
                $"PaddingPx は 0 以上で指定してください: {options.PaddingPx}",
                nameof(options));
        }
        ...
        {
            throw new ArgumentException(
                $"MaxDiscs は 1 以上で指定してください: {options.MaxDiscs}",
                nameof(options));
        }
    }
}
